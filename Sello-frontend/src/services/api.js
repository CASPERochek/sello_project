...
class ApiService {
    ...
    // ==================== ДАННЫЕ ИЗ SELLO ТОВАРОВ И НОВОСТЕЙ ====================
    
    // Получить товары из sello_tovar
    async getSelloProducts(filters = {}) {
        try {
            const params = new URLSearchParams(filters);
            const response = await fetch(`${API_BASE_URL}/constructor/api/sello/products/?${params}`, {
                headers: this.getAuthHeaders(),
            });
            
            console.log('Get sello products response status:', response.status);
            
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                console.warn('Ошибка получения товаров:', errorData);
                // Возвращаем пустой массив при ошибке
                return [];
            }
            
            const data = await response.json();
            console.log('Получены товары:', data.products ? data.products.length : 0);
            
            return data.products || [];
        } catch (error) {
            console.error('Ошибка получения товаров из sello_tovar:', error);
            return [];
        }
    }

    // Получить новости из sello_news
    async getSelloNews(filters = {}) {
        try {
            const params = new URLSearchParams(filters);
            const response = await fetch(`${API_BASE_URL}/constructor/api/sello/news/?${params}`, {
                headers: this.getAuthHeaders(),
            });
            
            console.log('Get sello news response status:', response.status);
            
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                console.warn('Ошибка получения новостей:', errorData);
                // Возвращаем пустой массив при ошибке
                return [];
            }
            
            const data = await response.json();
            console.log('Получены новости:', data.news ? data.news.length : 0);
            
            return data.news || [];
        } catch (error) {
            console.error('Ошибка получения новостей из sello_news:', error);
            return [];
        }
    }

    // Поиск контента
    async searchSelloContent(query, type = 'all') {
        try {
            const params = new URLSearchParams({ q: query, type });
            const response = await fetch(`${API_BASE_URL}/constructor/api/sello/search/?${params}`, {
                headers: this.getAuthHeaders(),
            });
    ...
    // Проверить подключение к Sello API
    async checkSelloConnection() {
        try {
            const url = `${API_BASE_URL}/constructor/api/sello/products/`;
            const response = await fetch(url, {
                method: 'GET',
                headers: this.getAuthHeaders(),
            });
            
            // Даже если ошибка авторизации, сервер работает
            return response.status !== 404;
        } catch (error) {
            console.warn('Нет подключения к Sello API:', error.message);
            return false;
        }
    }
    ...
    // Моковые товары для отладки
    getMockProducts() {
        return [
            {
                id: 1,
                title: 'Футболка хлопковая',
                image: 'https://via.placeholder.com/300x200/4a90e2/ffffff?text=Товар',
        ...
        return [
            {
                ...
                content: 'Мы рады представить нашу новую коллекцию осень-зима 2025.',
                category: 'Новости',
                type: 'news'
            },
        ];
