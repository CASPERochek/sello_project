...
const PreviewPage = () => {
  const location = useLocation();
  // 🔥 ИСПРАВЛЕНО: используем те же имена, что и при отправке
  const { blocks, text_color, bg_color } = location.state || {};
  ...
  const getTextStyle = (block) => {
    ...
    return {
      color: text_color || '#000000', // ← ИСПОЛЬЗУЕМ text_color
      textAlign: block.alignment || "left",
      fontSize,
      fontWeight,
      fontStyle,
  ...
  return (
    <div
      style={{
        backgroundColor: bg_color || '#ffffff', // ← ИСПОЛЬЗУЕМ bg_color
        padding: "20px",
        fontFamily: "Arial, sans-serif",
        minHeight: "100vh",
        position: "relative"
