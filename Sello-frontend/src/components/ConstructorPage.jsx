...
const ConstructorPage = () => {
  ...
  const [bgColor, setBgColor] = useState("#ffffff");
  const [savedDesigns, setSavedDesigns] = useState([]);
  const [currentDesignId, setCurrentDesignId] = useState(null); // ← ключевое состояние
  const [designName, setDesignName] = useState("Мой дизайн");
  const [isLoading, setIsLoading] = useState(false);
  const [showSaveModal, setShowSaveModal] = useState(false);
  const [showLoadModal, setShowLoadModal] = useState(false);
  ...
  // Функция сохранения в БД с JSON
  const saveToDatabase = async (name = designName) => {
    try {
      setIsLoading(true);
      ...
      // ОСНОВНОЕ ИЗМЕНЕНИЕ: используем ТОЛЬКО currentDesignId
      if (currentDesignId) {
        ...
        if (result && result.id) {
          setCurrentDesignId(result.id); // ← сразу сохраняем ID нового проекта
        }
      }
  ...
  const loadFromDatabase = async (id) => {
    try {
      ...
      setTextColor(design.text_color || "#000000");
      setBgColor(design.bg_color || "#ffffff");
      setCurrentDesignId(design.id); // ← КЛЮЧЕВО: устанавливаем ID
      setDesignName(design.name || "Мой дизайн");
  ...
  useEffect(() => {
    ...
    if (loadId) {
      ...
      if (!isNaN(idNum) && apiService.isAuthenticated()) {
        loadFromDatabase(idNum);
      } else if (loadId && !apiService.isAuthenticated()) {
        // Не авторизован — редирект на логин, но сохраняем loadId
        navigate(`/login?next=/constructor?load=${loadId}`);
      }
    }
    loadSavedDesigns();
  }, [location]);
